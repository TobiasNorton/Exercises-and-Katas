...
const recover = (string) => {
  ...
  letters.forEach((letter, index) => {
    for (let i = 0; i < numberKeys.length; i++) {
      const phrase = letters.slice(index, index + numberKeys[i].length)
      if (phrase.sort().join('') === numberKeys[i].split('').sort().join('')) {
        numbers.push(numberMap[numberKeys[i]].toString())
      }
    }    
  })
  return numbers.length ? numbers.join('') : 'No digits found'
}

...
function leaderboardSort(leaderboard, changes) {
  const changeObjects = changes.map(change => {
  ...
  changeObjects.forEach(changeObject => {
    const leaderboardIndex = leaderboard.indexOf(changeObject.name)
    const move = parseInt(changeObject.move)
    const removedName = leaderboard.splice(leaderboardIndex, 1)[0]
    leaderboard.splice(leaderboardIndex - move, 0, removedName)
  })
  return leaderboard
}
